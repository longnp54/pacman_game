...
//void Ghost::update(unsigned char i_level, Cell i_map[MAP_WIDTH][MAP_HEIGHT], Ghost& i_ghost_0, Pacman& i_pacman)
//{
void Ghost::update(unsigned char i_level, std::array<std::array<Cell, MAP_HEIGHT>, MAP_WIDTH>& i_map, Ghost& i_ghost_0, Pacman& i_pacman)
{
	...
	Cell imap2[MAP_WIDTH][MAP_HEIGHT];
		...
		if (0 == movement_mode) //The scatter mode
		{
			...
			switch (id)
			{
				case 0:
				{
					...
					target.x=CELL_SIZE * (MAP_WIDTH - 1);
					target.y=0;
					break;
				}
				case 1:
